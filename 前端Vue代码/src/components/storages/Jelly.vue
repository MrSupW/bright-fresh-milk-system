<template>
  <div class="main_container">
    <h1><i>This is the storage page of Jelly</i></h1>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="16">
      ...
      <el-table :data="jellyList" border stripe max-height="378" :highlight-current-row="true">
        ...
        <el-row :gutter="5">
          ...
          <el-col :span="8">
            <el-form-item label="Quantity" prop="quantity">
              <el-input autocomplete="off" type="age" placeholder = "Quantity..." v-model.number="editFormData.quantity" @keypress.native.enter="confirmEdit"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-dialog title="Add New" :visible.sync="addNewDialogVisible">
      <el-form :inline="true" ref="addNewFormRef" :rules="addNewFromDataRules" label-position="top" :model="addNewFromData">
        ...
        <el-row :gutter="5">
          ...
          <el-col :span="8">
            <el-form-item label="Quantity" prop="quantity">
              <el-input autocomplete="off" type="age" placeholder = "Quantity..." v-model.number="addNewFromData.quantity" @keypress.native.enter="confirmAddNew"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
  export default {
    ...
    methods:{
      refreshData(){
        if(this.isSearching) this.getJellySearchList();
        else this.getJellyList();
      },
      async getJellyList(){
        const info = {
        ...
        console.log(info)
        const res = await this.$http.post("getStorage/jelly",info);
        console.log('res',res)
        if(res.data.code === 0){
          this.jellyList = res.data.jellyList;
          this.total = res.data.total;
          console.log("data", res.data.jellyList)
          this.$message.success({message:res.data.msg,duration:1000})
        }else{
          this.$message.error(res.data.msg)
        }
      },
      async getJellySearchList(){
        const info = {
          pageSize :this.pageSize,
          currentPageIndex: this.currentPageIndex,
          searchContent: this.searchContent
        }
        console.log('getJellySearchListInfo',info)
        const res = await this.$http.post("searchStorage/jelly",info);
        console.log(' getJellySearchList res',res)
        if(res.data.code === 0){
          this.jellyList = res.data.jellyList;
          this.total = res.data.total;
          console.log("data", res.data.jellyList)
      ...
      async confirmDelete(){
        ...
        const res = await this.$http.post("/deleteStorage/jelly",info);
        if(res.data.code === 0){
          this.$message.success({message:res.data.msg,duration:1000})
          this.refreshData();
        }
        ...
        this.$refs.editFormRef.validate(async valid => {
          ...
          if(valid){
            if(await this.judgeCodeValidation(this.nowCode, this.editFormData.code)){
              const res = await this.$http.post("/editStorage/jelly",this.editFormData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000})
                this.refreshData();
              }
        ...
        this.$refs.addNewFormRef.validate(async valid => {
          if(valid){
            if(await this.judgeCodeValidation(null, this.addNewFromData.code)){
              ...
              const res = await this.$http.post("/addNewStorage/jelly",this.addNewFromData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000})
                this.refreshData();
              }
