<template>
  <div class="main_container">
    <h1><i>This is the storage page of PureMilk</i></h1>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="16">
  ...
  export default {
    ...
    methods:{
      ...
      async getPureMilkList(){
        ...
        console.log(info)
        const res = await this.$http.post("getStorage/pureMilk" +
            "",info);
        console.log('res',res)
        if(res.data.code === 0){
      ...
      async getPureMilkSearchList(){
        ...
        const res = await this.$http.post("searchStorage/pureMilk" +
            "",info);
      ...
      async confirmDelete(){
        ...
        const res = await this.$http.post("/deleteStorage/pureMilk" +
            "",info);
        if(res.data.code === 0){
          this.$message.success({message:res.data.msg,duration:1000});
          this.refreshData();
        ...
        this.$refs.editFormRef.validate(async valid => {
          ...
          if(valid){
            if(await this.judgeCodeValidation(this.nowCode, this.editFormData.code)){
              const res = await this.$http.post("/editStorage/pureMilk" +
                  "",this.editFormData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000});
                this.refreshData();
        ...
        this.$refs.addNewFormRef.validate(async valid => {
          if(valid){
            if(await this.judgeCodeValidation(null, this.addNewFromData.code)){
              ...
              const res = await this.$http.post("/addNewStorage/pureMilk" +
                  "",this.addNewFromData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000});
                this.refreshData();
