<template>
  <div class="main_container">
    <h1><i>This is the product page of MilkDrink</i></h1>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="16">
      ...
      <el-table :data="milkDrinkList" border stripe max-height="378" :highlight-current-row="true" >
        ...
        <el-table-column label="Flavor" prop="flavor"></el-table-column>
        <el-table-column label="Sugar" prop="sugar"></el-table-column>
    ...
    <el-dialog title="Edit" :visible.sync="editDialogVisible">
      <el-form :inline="true" ref="editFormRef" :rules="editFormRules" label-position="top" :model="editFormData">
        ...
        <el-row :gutter="5">
          ...
          <el-col :span="8">
            <el-form-item label="Flavor" prop="flavor">
              <el-input autocomplete="off" placeholder = "Flavor..." v-model="editFormData.flavor" @keypress.native.enter="confirmEdit"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Sugar" prop="sugar">
              <el-input autocomplete="off" placeholder = "Sugar..." v-model="editFormData.sugar" @keypress.native.enter="confirmEdit"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-dialog title="Add New" :visible.sync="addNewDialogVisible">
      <el-form :inline="true" ref="addNewFormRef" :rules="addNewFromDataRules" label-position="top" :model="addNewFromData">
        ...
        <el-row :gutter="5">
          ...
          <el-col :span="8">
            <el-form-item label="Flavor" prop="flavor">
              <el-input autocomplete="off" placeholder = "Flavor..." v-model="addNewFromData.flavor"  @keypress.native.enter="confirmAddNew"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Sugar" prop="sugar">
              <el-input autocomplete="off" placeholder = "Sugar..." v-model="addNewFromData.sugar"  @keypress.native.enter="confirmAddNew"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
  export default {
    ...
    data(){
      return{
        ...
        editFormRules: {
          ...
          flavor: [
            {required: true, message: 'Please enter the flavor!', trigger: 'blur'}
          ],
          sugar: [
            {required: true, message: 'Please enter the sugar!', trigger: 'blur'}
          ],
        },
        addNewFromDataRules:{
          code: [
    ...
    methods:{
      refreshData(){
        if(this.isSearching) this.getMilkDrinkSearchList();
        else this.getMilkDrinkList();
      },
      async getMilkDrinkList(){
        const info = {
          pageSize :this.pageSize,
          currentPageIndex: this.currentPageIndex
        }
        console.log(info)
        const res = await this.$http.post("getProducts/milkDrink",info);
        console.log('res',res)
        if(res.data.code === 0){
          this.milkDrinkList = res.data.milkDrinkList;
          this.total = res.data.total;
          console.log("data", res.data.milkDrinkList)
          this.$message.success({message:res.data.msg,duration:1000})
        }else{
          this.$message.error(res.data.msg)
        }
      },
      async getMilkDrinkSearchList(){
        const info = {
          pageSize :this.pageSize,
          currentPageIndex: this.currentPageIndex,
          searchContent: this.searchContent
        }
        console.log('getMilkDrinkSearchListInfo',info)
        const res = await this.$http.post("searchProducts/milkDrink",info);
        console.log(' getMilkDrinkSearchList res',res)
        if(res.data.code === 0){
          this.milkDrinkList = res.data.milkDrinkList;
          this.total = res.data.total;
          console.log("data", res.data.milkDrinkList)
      ...
      async confirmDelete(){
        ...
        const res = await this.$http.post("/deleteProduct/milkDrink",info);
        if(res.data.code === 0){
          this.$message.success({message:res.data.msg,duration:1000})
          this.refreshData();
        }
        ...
        this.$refs.editFormRef.validate(async valid => {
          ...
          if(valid){
            if(await this.judgeCodeValidation(this.nowCode, this.editFormData.code)){
              const res = await this.$http.post("/editProduct/milkDrink",this.editFormData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000})
                this.refreshData();
              }
        ...
        this.$refs.addNewFormRef.validate(async valid => {
          if(valid){
            if(await this.judgeCodeValidation(null, this.addNewFromData.code)){
              ...
              const res = await this.$http.post("/addNewProduct/milkDrink",this.addNewFromData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000})
                this.refreshData();
              }
