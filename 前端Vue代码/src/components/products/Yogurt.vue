<template>
  <div class="main_container">
    <h1><i>This is the product page of Yogurt</i></h1>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="16">
  ...
  export default {
    ...
    methods:{
      ...
      async getYogurtList(){
        ...
        console.log(info)
        const res = await this.$http.post("getProducts/yogurt",info);
        console.log('res',res)
        if(res.data.code === 0){
      ...
      async getYogurtSearchList(){
        ...
        const res = await this.$http.post("searchProducts/yogurt",info);
      ...
      async confirmDelete(){
        ...
        const res = await this.$http.post("/deleteProduct/yogurt",info);
        if(res.data.code === 0){
          this.$message.success({message:res.data.msg,duration:1000});
          this.refreshData();
        }
        ...
        this.$refs.editFormRef.validate(async valid => {
          ...
          if(valid){
            if(await this.judgeCodeValidation(this.nowCode, this.editFormData.code)){
              const res = await this.$http.post("/editProduct/yogurt",this.editFormData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000});
                this.refreshData();
              }
        ...
        this.$refs.addNewFormRef.validate(async valid => {
          if(valid){
            if(await this.judgeCodeValidation(null, this.addNewFromData.code)){
              ...
              const res = await this.$http.post("/addNewProduct/yogurt",this.addNewFromData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000});
                this.refreshData();
              }
