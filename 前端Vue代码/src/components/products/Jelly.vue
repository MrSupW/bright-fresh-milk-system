<template>
  <div class="main_container">
    <h1><i>This is the product page of Jelly</i></h1>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="16">
  ...
  export default {
    ...
    methods:{
      ...
      async getJellyList(){
        ...
        console.log(info)
        const res = await this.$http.post("getProducts/jelly",info);
        console.log('res',res)
        if(res.data.code === 0){
      ...
      async getJellySearchList(){
        ...
        const res = await this.$http.post("searchProducts/jelly",info);
      ...
      async confirmDelete(){
        ...
        const res = await this.$http.post("/deleteProduct/jelly",info);
        if(res.data.code === 0){
          this.$message.success({message:res.data.msg,duration:1000})
          this.refreshData();
        }
        ...
        this.$refs.editFormRef.validate(async valid => {
          ...
          if(valid){
            if(await this.judgeCodeValidation(this.nowCode, this.editFormData.code)){
              const res = await this.$http.post("/editProduct/jelly",this.editFormData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000})
                this.refreshData();
              }
        ...
        this.$refs.addNewFormRef.validate(async valid => {
          if(valid){
            if(await this.judgeCodeValidation(null, this.addNewFromData.code)){
              ...
              const res = await this.$http.post("/addNewProduct/jelly",this.addNewFromData);
              if(res.data.code === 0){
                this.$message.success({message:res.data.msg,duration:1000})
                this.refreshData();
              }
